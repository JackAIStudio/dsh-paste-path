...
官方 Lexical `PASTE_COMMAND` 会把剪贴板里 `kind=file` 的每一项都丢给 `intakeFiles` 上传。
目录 / macOS 包没有字节流，结果就是红框「上传失败」——这就是「拖进去可以、Cmd+C Cmd+V 不行」的根因。

所以 **⌘V 必须和 drop 同样分流**：目录/.app 插路径，图片和普通文件放行给官方。
只拦 `drop`、把粘贴留给 ⌘⇧V，等于把这条路又让给官方去失败。
...
2. **⌘V / Ctrl+V**：先在访达 `Cmd+C` 复制目录或 `.app`，再在输入框粘贴。和拖拽一样分流：目录变路径，图片/普通文件仍走官方附件。
3. **📎 文件选择器**：选中 `.app` 时同样会转成路径行（选择器选不了目录，系统限制）。
4. **⌘⇧V / Ctrl+Shift+V**：浏览器剪贴板里没有 `Files` 时的兜底（直接读系统 `NSPasteboard`）。
